...
class CreateDonorInstituteComponent extends Component {
  ...
  saveDonorInstitute = (event) => {
    ...
    if (this.state.id === "_add") {
      // create user
      DonorInstituteService.createDonorInstitute(dI)
        .then((res) => {
          const notify = () =>
            toast("Donör Kurumu başarılı bir şekilde kaydedildi.");
          notify();
          this.props.history.push("/donorinstitutes");
        })
        .catch((ex) => {
          const notify = () =>
            toast("Donör Kurumu kaydedilemedi. Hata Kodu: CNT-DON-INST-01");
          notify();
        });
    } else {
      DonorInstituteService.updateDonorInstitute(this.state.id, dI)
        .then((res) => {
          const notify = () =>
            toast("Donör Kurumu başarılı bir şekilde güncellendi.");
          notify();
          this.props.history.push("/donorinstitutes");
        })
        .catch((ex) => {
          const notify = () =>
            toast("Donör Kurumu güncellenemedi. Hata Kodu: CRT-DON-INS-02");
          notify();
        });
    }
              ...
              <div className="card-body">
                <form>
                  ...
                  <button
                    ...
                    onClick={this.saveDonorInstitute.bind(this)}
                    disabled={!this.state.isEditable}
                  >
                    {this.getButtonText()}{" "}
                  </button>
