...
class ListPermissionComponent extends Component {
        ...
        this.state = {
            ...
            columns: [
                ...
                },
                {
                    ...
                    return (
                        <div>
                            <button type="button" style={{marginRight:"10px"}} onClick={() => this.viewPermission(rowId)} className="btn btn-success">Görüntüle</button> 
                            <button type="button" style={{marginRight:"10px"}} onClick={() => this.updatePermission(rowId)} className="btn btn-info">Güncelle</button> 
    ...
    deletePermission(){
        ...
        PermissionService.deletePermission(this.state.selectedRowId).then(res => {
            PermissionService.getAllPermissions().then((res) => {
                this.setState({permissions : res.data});
            }).catch(ex => {
                console.error(ex);
    ...
    render() {
        const defaultSorted = [{
          ...
          const options = {
            ...
            {
              text: 'Tümü', value: this.state.permissions.length
            }] // A numeric array is also available. the purpose of above example is custom the text
          };
          
        return (
            <div className="container">
               <div class="col-sm-12 btn btn-info">Yetki Listesi</div> 
                <div>
                <ToolkitProvider
                    keyField="id"
                ...
                <Modal isOpen={this.state.showDeleteModal} toggle={this.handleShowModal}>
                    ...
                    <ModalFooter>
                        ...
                        <button className="btn btn-success" onClick={() => this.deletePermission()}>Sil</button>
                    </ModalFooter>
                </Modal>     
            </div>
            )
