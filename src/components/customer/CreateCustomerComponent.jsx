  ...
  saveCustomer = (event) => {
    ...
    if (this.state.id === "_add") {
      CustomerService.createCustomer(customer)
        .then((res) => {
          const notify = () =>
            toast("Müşteri başarılı bir şekilde kaydedildi.");
          notify();
          this.props.history.push("/customers");
        })
        .catch((ex) => {
          const notify = () =>
            toast("Müşteri kaydedilemedi. Hata Kodu: CRT-CST-01");
          notify();
        });
    } else {
      CustomerService.updateCustomer(this.state.id, customer)
        ...
        .catch((ex) => {
          const notify = () =>
            toast("Müşteri güncellenemedi. Hata Kodu: CRT-CST-02");
          notify();
        });
    }
    // If opened as modal
                ...
                <form>
                  ...
                  <div className="form-group">
                    ...
                    <select
                      ...
                      onChange={this.changeCustomerTypeHandler}
                      disabled={!this.state.isEditable}
                    >
